                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-8 text-white">
                            <div className="max-w-3xl mx-auto">
                                <div className="flex justify-between items-end">
                                    <div>
                                        <span className="bg-gradient-to-t from-brand-dark to-brand-light text-white px-3 py-1 rounded-full text-sm font-bold mb-3 inline-block shadow-sm">
                                            Special Offer
                                        </span>
                                        ...
                                        <p className="text-3xl font-bold bg-gradient-to-t from-brand-dark to-brand-light bg-clip-text text-transparent">
                                            ${item.price.toFixed(2)}
                                        </p>
                                    </div>
                                    <button
                                        onClick={() => onAddToCart(item)}
                                        className="bg-white text-primary-500 hover:bg-primary-50 px-6 py-3 rounded-full font-bold text-lg transition-colors flex items-center gap-2"
                                    >
                    ...
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                        {items.map((_, index) => (
                            <button
                                ...
                                className={`w-3 h-3 rounded-full transition-all ${index === currentIndex ? 'bg-primary-500 w-8' : 'bg-white/50'
                                    }`}
                            />
                        ))}
                    </div>
